		...
		success: function(data){					
			$.each(data, function(index, element) {			
				if(element.QtbNumber !== 'TOM_AUTOFILL'){
					$tableCode += "<tr class='usersTableElements' userId='"+element.Id+"'><td><b>"+element.FirstName+" "+element.LastName+" ("+element.QtbNumber+")</b> <span style='float:right;'>(Id="+element.Id+")</span></td></tr>";				
				}
			});		
...
function initUserPopovers(data){
	$.each(data, function(index, element) {		
		...
		$userPopoverContent += "</div>";
		$userPopoverContent += "<div class='form-group'><label class='control-label col-sm-4' style='text-align:left;' for='qtbNumber'>Qtb number:</label>";
		$userPopoverContent += "<div class='col-sm-8'><input type='text' class='form-control' id='qtbNumber' value='"+element.QtbNumber+"'></div>";
		$userPopoverContent += "</div>";
		...
		$( ".usersTableElements[userId='"+element.Id+"']" ).popover({
			...
			$('.buttonSaveUser').click(function(){
				var $userInfo = {
					...
					qtbNumber: 	$(this).parents('.popover-content').find('#qtbNumber').val().trim().toLowerCase(),
...
function  initNewUserButton(){
	$('#addNewUser').click(function(){
		...
		$createUserForm += "</div>";
		$createUserForm += "<div class='form-group'><label class='control-label col-sm-4' style='text-align:left;' for='qtbNumber'>Qtb number:</label>";
		$createUserForm += "<div class='col-sm-8'><input type='text' class='form-control' id='qtbNumber' placeholder='qtb1234'></div>";
		$createUserForm += "</div>";
			...
			buttons: 	[{
							...
							action: 	function(dialogItself) {
											var $newUserInfo = {
												...
												qtbNumber: 	dialogItself.getModalBody().find('#qtbNumber').val().trim().toLowerCase(),
