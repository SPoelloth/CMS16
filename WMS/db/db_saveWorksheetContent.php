...
<?php
	...
	$insertMultiQuery = "DELETE FROM `worksheet_content` WHERE WorksheetId=" . $mysqli->real_escape_string($lastElement['worksheetId']) . " AND TemplateRowId=" . $mysqli->real_escape_string($lastElement['templateRowId']) . " AND ResponsiblePersonId <> (SELECT Id FROM user WHERE Username='TOM_AUTOFILL'); ";
	...
	foreach($worksheetContents as $content) {
		...
		$insertMultiQuery .= "(SELECT Id FROM `user` WHERE Username = '" . $mysqli->real_escape_string($content['responsiblePersonQtbNumber']) . "'), ";
